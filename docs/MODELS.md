...
| [QuickQual](models/quickqual.md) | Good / Usable / Bad probabilities | Frozen DenseNet121 features plus an SVM | short side 512, aspect kept (no mask) | EyeQ (SVM fitted); ImageNet (frozen features) | Yes | Not applicable — nothing is trained | None stated | — | 2023-11 | 2026-09-15 |
| [QuickQual-MEME](models/quickqual-meme.md) | One probability that the photograph is bad — **no classes** | Frozen DenseNet121, nine features, a nine-weight linear model | short side 512, aspect kept (no mask) | EyeQ (parameters fitted); ImageNet (frozen features) | Yes — ten numbers printed in the README | No, and the feature selection is not published | None stated | [AutoMorphalyzer](projects/automorphalyzer.md) | 2023-11 | 2026-09-15 |
...
| [LightVesselNet](models/lightvesselnet.md) | Blood vessels, one class | Encoder–bottleneck–decoder, depthwise-separable with SE and multi-scale attention, **75k parameters** | **Native, no resize in the code** — the paper says 512² | DRIVE, STARE, CHASE-DB1, FIVES, HRF | **No** | Yes — one notebook, `EPOCHS = 1` as committed | **None stated** | — | 2026-06 | 2026-09-19 |
| [LCNet](models/lcnet.md) | Blood vessels, one class | U-shaped, depthwise-separable, coordinate attention and ASPP, 2.65M parameters | **Unknown** — no code located | DRIVE, STARE, CHASE-DB1, IOSTAR | **No** | **No — none located** | Not applicable | — | Not applicable | 2026-09-19 |
| [LHU-VT](models/lhu-vt.md) | Blood vessels, one class | U-Net with **octonion** (hypercomplex) convolutions, trained with a vessel-thickness-weighted Dice loss | **Unknown** — no code located | CHASE-DB1, DRIVE, STARE, HRF | **No** | **No — none located** | Not applicable | — | Not applicable | 2026-09-19 |
...
| Model | Produces | Architecture | Grid (in → out) | Trained on | Weights | Training code | License | Used by | Last commit | Last checked |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| [VascX disc](models/vascx-disc.md) | Optic disc (no cup) | U-Net ensemble | 1024² | 15+ published datasets plus Rotterdam Study images | Yes | No | Code not stated; weights AGPL-3.0 | [VascX](projects/vascx.md) | 2026-08 | 2026-09-10 |
| [AutoMorph disc-and-cup](models/automorph-disc-cup.md) | Optic disc **and cup** | W-Net retrained for this task, ensemble of 8 | 512² → back to original | REFUGE (800), GAMMA (100) | Yes | No — architecture's training code is in lwnet | Apache-2.0 (host); MIT architecture | [AutoMorph](projects/automorph.md), [AutoMorphalyzer](projects/automorphalyzer.md), [AutoMorphClass](projects/automorphclass.md) | 2025-06 | 2026-09-10 |
...
- **Purpose groups** — every model belongs to exactly one of five: quality, vessels, artery/vein,
  ...
  one row per release.
- **Grid (in → out)** — the pixel raster the model works on: the size the image is resampled to
  before inference, and the size of the mask that comes back. It is the most consequential column
  for comparing models, because every biomarker measured in pixels is measured on this grid: a
  4-pixel vessel width means one thing at 512 and another at 1472, and two models' masks cannot be
  compared pixel-for-pixel unless their grids match. `→ back to original` means the code resamples
  the mask to the input photograph's own dimensions before returning it. Each model page records
  where the grid is set in code, because papers and model cards usually omit it.
