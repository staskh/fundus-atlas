...
```bash
# the archive is link-shared on Kaggle: download it by hand first
uv run python -m datasets.reyia --archive ~/Downloads/archive.zip
uv run python -m datasets.fetch_um_resolution --dataset reyia
```

**The Kaggle link is share-only** — the API answers 403 — so there is no unattended route and the
fetcher requires `--archive`.

**Most of that archive is not REYIA.** Of its 10,075 files, **7,712 are synthetic photographs**
generated by [RLAD](../models/rlad.md), the diffusion model from the same group: sixteen
generations of one training split. Nothing measured on a generated image says anything about an eye,
so they are skipped, as is the ultra-wide-field `AVWIDE` subset. What is built is **563
photographs** across eight subsets, each named for the collection its images came from.

Each photograph arrives with its annotation **twice** — as an RGB label map and as binary artery and
vein masks — so the fetcher reads the label map and then checks its own output against the published
binaries, writing the result into `build.json`. On 1,126 masks the two agreed everywhere but four
photographs (section 7).

REYIA marks a crossing **magenta**, where Fundus-AVSeg and HRF-AV both use green, which is why every
dataset declares its own palette rather than sharing one.

The photographs come from nine other collections and the authors publish no scale, so
`fetch_um_resolution` measures each subset separately — eight cameras, eight figures.
...
- **Four photographs carry two annotations that contradict each other.** *Our finding, 2026-09-17:*
  the archive publishes every annotation twice, as an RGB label map and as binary artery and vein
  masks, and the fetcher checks one against the other. On 1,126 masks they agree everywhere except
  `MAGREB/Fimage31prime` — where they differ by 124,677 artery pixels on a frame whose whole vessel
  network is about 60,000 — and three MBRSET photographs, `256_3`, `296_1` and `303_1`, differing by
  10,000 to 36,000. Both files are the right size and belong to the photograph, so one of them
  annotates something else. Nothing available says which, so all four are recorded in
  `src/datasets/exclusions/reyia.json` and their maps are not trusted.
- **Three quarters of the archive is synthetic.** 7,712 of its 10,075 files are
  [RLAD](../models/rlad.md)-generated images rather than photographs — sixteen generations of one
  training split. They are not built, and nothing in this catalogue may be measured on them.
- **MAGREB's discs scatter too widely to give it a scale.** Its 32 sampled optic discs disagree by
  14.9% of their median, above the tenth this atlas accepts as one camera, so those 69 photographs
  carry no microns-per-pixel figure. Every other subset has one.
