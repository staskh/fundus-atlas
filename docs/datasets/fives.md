...
| Microns per pixel | Not published. **Inferred: 5.951 µm/px** — a camera scale from the median optic disc of 32 photographs, [evidence](../../results/um_resolution/fives.json). Good to roughly a tenth (§9 of the `fetch-um-resolution` skill); not a per-eye calibration |
