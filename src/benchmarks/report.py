...
def write_docs(
    ...
    path.write_text(
        "\n".join(_docs(benchmark, scored, missing_models, missing_datasets, columns)) + "\n"
    )
    return path
...
def _docs(
    ...
    yield (
        ...
        "It is generated by the run, so a column added to the evidence and not explained here is a "
        "bug rather than an omission. What came out is a separate page: "
    ...
    yield "| --- | --- | --- | --- | --- | --- |"
    for model in models:
        declared = _first(scored, model=model)["declared"]
        yield (
            f"| [{model}](../models/{model}.md) | {_pin(declared.get('upstream', {}))} | "
    ...
    for dataset in datasets:
        entry = _first(scored, dataset=dataset)
        counts = entry["counts"]
        yield (
            f"| [{dataset}](../datasets/{dataset}.md) | {counts['total']} | "
            f"{', '.join(entry.get('grade_source') or ['—'])} | "
            f"{_number(entry.get('padding'))} | {_excluded(counts['excluded'])} |"
        )
    ...
    yield f"python -m benchmarks --benchmark {benchmark} --model {models[0]} --dataset {datasets[0]}"
